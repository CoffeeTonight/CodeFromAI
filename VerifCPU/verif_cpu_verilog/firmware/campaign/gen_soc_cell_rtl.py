...
def build_soc_cell_source(only_keys: frozenset[str] | None = None) -> tuple[list[str], int]:
    lines = [
...
def write_soc_cell(
    ...
    write_widths: bool = True,
) -> int:
    lines, n = build_soc_cell_source(only_keys)
    ...
    scope = f"keys={','.join(sorted(only_keys))}" if only_keys else "all"
    print(f"[soc_cell] Wrote {out} ({n} bus cell module(s), {scope})")
    return n
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Generate verif_vcpu_soc_cell*.v")
    ap.add_argument(
        "--only",
        default="",
        help="Comma-separated bus keys (e.g. apb3,ahb_lite,axi4lite) for tier-scoped cell RTL",
    )
    ap.add_argument(
        "--out",
        type=Path,
    ...
    ap.add_argument(
        "--no-widths",
        action="store_true",
        help="Skip regenerating include/verif_bus_soc_widths.vh",
    )
    args = ap.parse_args()
    only: frozenset[str] | None = None
    if args.only.strip():
        only = frozenset(k.strip() for k in args.only.split(",") if k.strip())
    write_soc_cell(args.out, only_keys=only, write_widths=not args.no_widths)
    return 0
