"""Combination-matrix regression: all parse axes via real ``scan-inst`` CLI."""
...
# Nodes that ``scan-inst`` hierarchy + path-walk must recognize.
...
def _scan_inst_hierarchy_paths(tmp_path: Path, *, fixture: Path = _FIXTURE) -> set[str]:
...
def test_scan_inst_hierarchy_matrix_nodes(tmp_path: Path):
    found = _scan_inst_hierarchy_paths(tmp_path)
    missing = _EXPECTED_HIERARCHY - found
    assert not missing, f"scan-inst hierarchy missing: {sorted(missing)}\nfound={sorted(found)}"
    for bad in _ABSENT_NODES:
        assert bad not in found


def test_scan_inst_path_walk_matrix_nodes(tmp_path: Path):
...
def test_scan_inst_hierarchy_extreme_newline_split(tmp_path: Path):
    """Every token on its own line: ``ASD\\n u_asd\\n(\\n.a\\n(\\nw\\n)`` etc."""
    found = _scan_inst_hierarchy_paths(tmp_path, fixture=_NEWLINE_FIXTURE)
    missing = _NEWLINE_EXPECTED - found
    assert not missing, (
        f"scan-inst hierarchy missing newline-split nodes: {sorted(missing)}\n"
        f"found={sorted(found)}"
    )


def test_scan_inst_path_walk_extreme_newline_split(tmp_path: Path):
