...
from scan_inst.connectivity import run_connectivity_request
from scan_inst.elab import elaborate
from scan_inst.index import DesignIndex
from scan_inst.validate_connect import (
    OracleKind,
...
def test_compare_detects_fp_risk():
    ...
    from scan_inst.connectivity import ConnectResult
    from scan_inst.models import ConnectEndpoint

    def _res(connected: bool) -> ConnectResult:
        ep = ConnectEndpoint(spec="a", kind="port")
        epb = ConnectEndpoint(spec="b", kind="port")
...
def test_unified_verify_strict_oracle():
    from scan_inst.cache import load_or_build_index
