...
> MD = **무엇을** 검증할지. `ops/static/coi_conn.py` = **scan_inst** 호출 등 구체 실행 (crystallize).
...
| 항목 | 원칙 |
|------|------|
| 도구 | [scan_inst](/home/user/Desktop/scan_inst) — `check-connect` / `check-connect-batch` |
...
- instance 목록 재생성: `scan-inst <filelist> --top <TOP> -o instances.tsv`
...
```bash
pip install -e /home/user/Desktop/scan_inst
cd "$RTL_ROOT"   # discovered rtl_subdir under workspace clone
```
...
| H2 | APB periphery | `chip_top_example.u_periph_sfr` | SFR 슬레이브 |
| H3 | agent / cell | `chip_top_example` 하위 `u_ag_1` / `u_bus` (gen VH) | SCPU·버스 셀 |

`scan-inst … -o instances.tsv` 로 실제 `full_path` 확인 후 endpoint 기입.
...
> `expected_connected`는 **gate 전용** — scan_inst에 넘기기 전 ops가 strip. TSV `connected`와 비교.

---

## conn_example.json — scan_inst 입력 예제 (모든 경우)
...
scan_inst 번들 RTL — repo root `/home/user/Desktop/scan_inst` 에서 실행:
...
원본: `scan_inst/examples/stress_seed42/stress_42_d8.connect.json`
...
```bash
scan-inst <filelist> --top <TOP> --index-cwd "$RTL_ROOT" \
...
```bash
scan-inst filelists/eda/test/chip_top_example/manifest.list \
  --top chip_top_example \
  --index-cwd "$RTL_ROOT" \
...
- 도구 설치 확인: `conn_example.json` → `reference_stress` 케이스, cwd=`/home/user/Desktop/scan_inst`

### 다른 SoC 스타일

- top/filelist가 VCS `-F` / 사내 `soc.f` 이어도 동일 원칙
- endpoint 표기만 바뀜 — ops가 `scan-inst --help-connect` 기준으로 JSON 생성
- SpyGlass/formal 등 다른 static 도구 병행 가능하나 **이 그룹의 COI 판정 주체는 scan_inst**