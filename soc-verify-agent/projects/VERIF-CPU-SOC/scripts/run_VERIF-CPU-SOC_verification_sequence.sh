...
log "Update reports/index.yaml run_id paths to these runs if reports should reference them:"
log "  Step 1: runs/${RUN_ID_PREFIX}-01_sanity_VerifCPU_c-compile_and_elab"
log "  Step 2: runs/${RUN_ID_PREFIX}-02_static_COI_connectivity_chip_top"
log "  Step 3: runs/${RUN_ID_PREFIX}-03_simulation_slave_R_W_single_burst_cpu_sync"
log "Then re-run: ./scripts/99_generate_verification_reports.sh"