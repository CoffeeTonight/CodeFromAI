...
| 항목 | 원칙 |
|------|------|
| 도구 | `~/tools/__CFI/scan_inst` — `check-connect` / `check-connect-batch` |
...
```bash
pip install -e ~/tools/__CFI/scan_inst
...
scan_inst 번들 RTL — repo root `~/tools/__CFI/scan_inst` 에서 실행:
...
- 도구 설치 확인: `conn_example.json` → `reference_stress` 케이스, cwd=`~/tools/__CFI/scan_inst`
