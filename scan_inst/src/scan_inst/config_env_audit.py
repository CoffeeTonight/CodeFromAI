"""Log JSON ``env`` blocks and effective scan-inst behavioral environment."""
...
def _index_ifdef_policy_line() -> str:
    if not lazy_processing_enabled():
        return (
            "active-at-index (SCAN_INST_LAZY=0 eager mode; "
            "ifndef branches filtered per defines)"
        )
    if lazy_index_ifdef():
        return (
            "active-at-index (SCAN_INST_LAZY_IFDEF=1; "
...
def format_config_env_audit_lines(
    ...
    *,
    ...
    Call after :func:`scan_inst.run_request.apply_config_env_from_document`.
    """
    ...
    lines.append("config-env: behavioral SCAN_INST_* / HCH_INDEX_CWD (effective):")
