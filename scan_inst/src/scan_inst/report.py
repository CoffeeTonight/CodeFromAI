"""End-of-run summary report for scan-inst."""
...
def default_log_path(filelist_path: str, output_path: str = "-") -> Path:
    ...
    if output_path != "-":
        ...
        return out.parent / f"{out.name}.scan-inst.log"
    fl = Path(filelist_path).resolve()
    return fl.parent / f"{fl.stem}.scan-inst.log"
    ...
    *,
    ...
    with log_path.open(mode, encoding="utf-8") as fh:
        if append:
            ...
            fh.write(f"# scan-inst run {stamp}\n")
        fh.write("\n".join(lines))
        fh.write("\n")
