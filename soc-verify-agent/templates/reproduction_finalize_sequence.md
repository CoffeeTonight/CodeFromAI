...
- `run_{{project_id}}_verification_sequence.sh` — **no CLI args**, bash steps in `verification_sequence.yaml` order
...
## Deliverables
1. `scripts/run_{{project_id}}_verification_sequence.sh`
...
5. `runs/orchestrator/{{run_id}}/reproduction_sequence_finalize.json`
