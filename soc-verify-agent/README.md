...
### Three layers (remember this)

| Layer | What | Who changes it |
|-------|------|----------------|
| **Platform** | LangGraph, trust, node_gate, loop_guard, CLI | `src/soc_verify/`, `registry/policies.yaml` — 공통 |
| **Workspace** | Confluence·git·JIRA·스케줄·LLM endpoint | 루트 `config.json` — 팀마다 |
| **Verification** | PASS/FAIL “법” | `projects/{id}/verification/{stage}/{group}/` **MD only** |

검증 **판정**은 CHECK·RESPOND·MILESTONE MD만 따릅니다. Python `ops/`는 실행 crystallization이지 판정 근거가 아닙니다.

### 검증 방법론 (SKILL)

- **1방법론 = 1파일:** `projects/{id}/skills/{skill_id}/SKILL.md`
- frontmatter에 `milestone` (필수), `stage`+`group` (gate 연결 시) 명시
- `setup_group` → materialize → `verification/…` MD + (선택) `verify_smoke` 1바퀴  
  규칙: [`registry/skill_materialize_spec.yaml`](registry/skill_materialize_spec.yaml)

### LangGraph 3종

| Graph | When | Entry |
|-------|------|--------|
| `setup_group` | 과제 onboarding — SKILL·초보자 스크립트 | `soc-verify graph start --graph setup_group --project ID` |
| `verify_group` | 단일 gate 실행·평가·승격 | `--graph verify_group --stage ST --group G` |
| `orchestrator` | acquisition + due verify 큐 | `soc-verify run` |

노드·엣지 SSOT: [`registry/graph_flow_spec.yaml`](registry/graph_flow_spec.yaml)  
요약 다이어그램: [`templates/obsidian/11-LANGGRAPH-SUMMARY.md`](templates/obsidian/11-LANGGRAPH-SUMMARY.md)

### 사람이 처음 읽을 순서

1. [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) — end-to-end phase 표
2. [`registry/graph_flow_spec.yaml`](registry/graph_flow_spec.yaml) — 그래프 계약
3. [`templates/obsidian/02-LAYERS.md`](templates/obsidian/02-LAYERS.md) — 레이어
4. [`templates/obsidian/08-RUNNER-LOOP.md`](templates/obsidian/08-RUNNER-LOOP.md) — trust·parity·promote
5. 예시 gate: `projects/EXAMPLE-SOC/verification/simulation/gpio_ext/CHECK.md`
...
soc-verify skill add MY-SOC --file path/to/SKILL.md
...
## Using with an LLM (company agent)

회사 LLM에게 “이 레포를 이해하고 운용해라”고 할 때의 SSOT입니다.  
역할별 MOC: [`templates/obsidian/00-HUB.md`](templates/obsidian/00-HUB.md)

### Official contract

[`registry/graph_flow_spec.yaml`](registry/graph_flow_spec.yaml) 하단 `llm_contract:`:

| Read | Never read (for verify logic) |
|------|-------------------------------|
| `registry/graph_flow_spec.yaml` | `src/soc_verify/graphs/*.py` |
| `templates/obsidian/08-RUNNER-LOOP.md` | `registry/policies.yaml` (판정 임의 변경) |
| `runs/{run_id}/md_only_prompt.md` | |
| `registry/node_contract.yaml` (sandbox·exit) | |
| `templates/scripts/README.md` (reproduction) | |

그래프 진행: 노드 완료마다 **`graph tick`** (또는 HTTP `POST …/tick`). exit 산출물 없으면 tick 차단.

### Role-based reading

| Role | Start here | Then read |
|------|------------|-----------|
| **verify sub-agent** | [`templates/obsidian/SUB_AGENT.md`](templates/obsidian/SUB_AGENT.md) | `md_only_prompt.md` → CHECK/RESPOND only |
| **setup_group** | `runs/setup/{run_id}/read_catalog.json` | catalog paths → sandbox `read_file` on SKILL + verification MD |
| **orchestrator** | [`templates/obsidian/ORCHESTRATOR.md`](templates/obsidian/ORCHESTRATOR.md) | observe `verdict_*.json`, never self-judge PASS |
| **VCPU→SoC 통합** | [`templates/obsidian/agent/vcpu-soc-integration/00-INTEGRATION-HUB.md`](templates/obsidian/agent/vcpu-soc-integration/00-INTEGRATION-HUB.md) | (아래 VERIF-CPU-SOC 절) |

Setup LLM system prompt: [`templates/llm/system_setup_adaptive.txt`](templates/llm/system_setup_adaptive.txt)

### Copy-paste briefing (give your LLM)

```
You operate soc-verify-agent at {ROOT}.

Understand:
1. docs/ARCHITECTURE.md — platform / config / verification MD
2. registry/graph_flow_spec.yaml — graphs + llm_contract
3. templates/obsidian/00-HUB.md — role entry
4. For verify: SUB_AGENT.md + runs/{run_id}/md_only_prompt.md only for PASS/FAIL
5. For setup: runs/setup/{run_id}/read_catalog.json — read listed SKILL and verification paths

Rules:
- Advance with: soc-verify --root {ROOT} graph tick --session {SESSION_ID}
- Never use src/soc_verify/graphs/*.py for verification decisions
- Reproduction scripts: no gate CLI flags (templates/scripts/README.md)
```

### LLM API wiring

`config.json` → `llm` 블록 (`soc-verify setup llm`). `invoke-llm` / `graph tick` 시 플랫폼이 flow_spec + node_sandbox + (setup 시) `setup_read_catalog`를 payload로 전달합니다.
...
- **Obsidian vault**: `templates/obsidian/` → 사용자 vault `05-Agents/` — MOC·역할별 가이드는 위 [**Using with an LLM**](#using-with-an-llm-company-agent) 및 [[00-HUB]](templates/obsidian/00-HUB.md)
- **Sub-agent**: `sub_stop.json` on fail-fast; PASS only with `verdict_*.json`
...
| `~/tools/__CFI/hierwalk` | COI gate (`hier-walk`) |
