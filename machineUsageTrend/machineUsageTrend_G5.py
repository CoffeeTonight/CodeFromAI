...
class DataProcessor:
    def __init__(self, csv_file=None, output_dir="OUTPUT_TEST",
        ...
        if not os.path.exists(self.base_weekly_file):
            default_df = pd.DataFrame(columns=["Week", "Sum of Avr. Monthly Used Machine"])
    ...
    def _create_minimal_config(self):
        config = {
            "Peak Weekly Used Machine": {"type": "equation", "equation": "MAX(Sum of Avr. Monthly Used Machine per week)"},
            "Avr. Monthly Used Machine Ratio (%)": {"type": "equation", "equation": "(Sum of Avr. Monthly Used Machine / Machine Capacity) * 100 (%)"},
            "{col}_Ratio": {"type": "equation", "equation": "{col} / Machine Capacity"},
            "{col}_Share": {"type": "equation", "equation": "{col} / Sum of Avr. Monthly Used Machine"}
        }
    ...
    def generate_all_weeks_data(self, start_date="2022-01-01", end_date="2025-03-31", base_cols="A:I"):
        ...
        df = df[["Week", "Year", "Year-Month", "Machine Capacity"] + self.base_cols + ["Sum of Avr. Monthly Used Machine"]]
    ...
    def load_and_convert_data(self):
        ...
        self.df.columns = ["Year-Month", "Year", "Machine Capacity", "Week"] + [col.capitalize() for col in self.base_cols] + ["Sum of Avr. Monthly Used Machine"]
    ...
    def calculate_columns(self):
        ...
        self.df["Avr. Monthly Used Machine Ratio (%)"] = (self.df["Sum of Avr. Monthly Used Machine"] / self.df["Machine Capacity"]) * 100
    ...
    def convert_to_monthly(self):
        ...
        agg_dict = {
            ...
            "Sum of Avr. Monthly Used Machine": "mean",
            "Avr. Monthly Used Machine Ratio (%)": "mean",
        }
        for col in self.base_cols:
        ...
        # Calculate Peak Weekly Used Machine as max of weekly sums per month
        peak_values = []
        for year_month in monthly_df["Year-Month"]:
            weekly_sums = self.df[self.df["Year-Month"] == year_month]["Sum of Avr. Monthly Used Machine"]
            if not weekly_sums.empty:
                ...
                if peak_value in weekly_sums.values:
                    ...
                    print(f"Warning: Adjusted Peak Weekly Used Machine for {year_month} to {closest_value} from {peak_value}")
            else:
                peak_values.append(0)
        monthly_df["Peak Weekly Used Machine"] = peak_values

        # Move "Peak Weekly Used Machine" next to "Avr. Monthly Used Machine Ratio (%)"
        cols = monthly_df.columns.tolist()
        avr_idx = cols.index("Avr. Monthly Used Machine Ratio (%)")
        cols.pop(cols.index("Peak Weekly Used Machine"))
        cols.insert(avr_idx + 1, "Peak Weekly Used Machine")
    ...
    def save_combined_html(self):
        if os.path.exists(self.base_monthly_file):
            ...
            print(f"Debug: Sample data: {df[['Year-Month', 'Sum of Avr. Monthly Used Machine']].head()}")
        else:
        ...
        # First Graph
        fig1 = go.Figure()
        fig1.add_trace(go.Bar(x=df["X_Axis"], y=df["Sum of Avr. Monthly Used Machine"], name="Sum of Avr. Monthly Machine Operation",
        ...
        for i, col in enumerate(base_cols_cap):
            if col in df.columns:
                ...
                fig1.add_trace(go.Scatter(x=df["X_Axis"], y=ratio_values, name=f"{col} (%)", mode="lines",
                                          line=dict(color=colors[i % len(colors)], width=3), opacity=1, yaxis="y2"))
        ...
        if latest_idx is not None:
            value1 = df["Sum of Avr. Monthly Used Machine"].iloc[latest_1_idx]
            value = df["Sum of Avr. Monthly Used Machine"].iloc[latest_idx]
        ...
        fig1.update_layout(
            ...
            yaxis=dict(title=dict(text="Sum of Avr. Monthly Machine Operation", font=dict(size=20)), tickfont=dict(size=20)),
            yaxis2=dict(
        ...
        for i, col in enumerate(base_cols_cap):
            if col in df.columns:
                ...
                fig2.add_trace(go.Bar(x=df["X_Axis"], y=df[col], name=f"{col}", marker_color=bar_color, opacity=1))
        ...
        fig2.update_layout(
            ...
            yaxis=dict(
                title=dict(text="Average Machine Usage", font=dict(size=20)),
                tickfont=dict(size=20),
        ...
        df_table = df_table.rename(columns={"Sum of Avr. Monthly Used Machine": "Sum of Avr. Monthly Used Machine",
                                            "Avr. Monthly Used Machine Ratio (%)": "Avr. Monthly Used Machine Ratio (%)",
                                            "Peak Weekly Used Machine": "Peak Weekly Used Machine"})
        html_content = f"""
        <!DOCTYPE html>
        <html>
        <head>
    ...
    def save_heatmap_html(self):
        ...
        df = df.fillna(0)
        heatmap_data = df[base_cols_cap].div(df["Sum of Avr. Monthly Used Machine"], axis=0).fillna(0) * 100
    ...
    def update_excel_sheet(self):
        if os.path.exists(self.base_weekly_file):
            ...
            base_weekly_df = pd.DataFrame(columns=["Year-Month", "Machine Capacity", "Week", "Sum of Avr. Monthly Used Machine"])
        ...
        if self.df is not None:
            ...
            if not common_cols_weekly:
                common_cols_weekly = ["Year-Month", "Machine Capacity", "Week", "Sum of Avr. Monthly Used Machine"]
