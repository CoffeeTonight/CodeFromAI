...
index so large SoCs can be validated without SCAN_INST_LAZY=0 on every file.
"""
...
from scan_inst.connect_endpoints import parse_connect_endpoint
from scan_inst.connect_request import ConnectivityCheck, ConnectivityRequest, load_connect_request
from scan_inst.connectivity import ConnectResult, ConnectivityBatchResult, run_connectivity_request
from scan_inst.elab import elaborate
from scan_inst.filelist import parse_filelist
from scan_inst.index import DesignIndex
from scan_inst.lazy_scope import elab_scope_paths, endpoint_specs_from_request, lazy_scoped_connect_elab
from scan_inst.models import FlatRow
from scan_inst.progress import progress_callback
from scan_inst.run_request import apply_config_env_from_document, jobs_from_env
...
def _run_eager_files_oracle(
    ...
    *,
    ...
    prev_lazy = os.environ.get("SCAN_INST_LAZY")
    os.environ["SCAN_INST_LAZY"] = "0"
    try:
        ...
        if prev_lazy is None:
            os.environ.pop("SCAN_INST_LAZY", None)
        else:
            os.environ["SCAN_INST_LAZY"] = prev_lazy
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    from scan_inst.cache import default_cache_dir, load_or_build_index
    from scan_inst.elab import elaborate_tops_parallel
    from scan_inst.lazy_scope import lazy_filelist_defer_exists
    from scan_inst.perf import effective_low_memory
    from scan_inst.progress import ProgressReporter
