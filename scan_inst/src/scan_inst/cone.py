...
from scan_inst.connect_scan import (
    ...
    scan_ff_endpoint_sets,
)
from scan_inst.connectivity import resolve_endpoint
...
def _build_cone_module_index(
    ...
    *,
    ...
    ff_d_raw, ff_q_raw = scan_ff_endpoint_sets(text, param_map=pmap)
    ff_d_reps = frozenset(
        {comb.net_rep.get(n, n) for n in ff_d_raw}
    )
    ff_q_reps = frozenset(
        {comb.net_rep.get(n, n) for n in ff_q_raw}
    )
