...
Open project: scan_inst/  (mark ``src`` as Sources Root if imports fail)

Run configurations (``.run/``):
  - debug waypoint fanout          → this script, breakpoint on demand
  - scan-inst waypoint example     → CLI flat JSON
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    ap.add_argument(
        "--check-id",
        default="wp_drv_to_child",
        help="which check id to run from JSON (default: wp_drv_to_child)",
    )
    ap.add_argument(
        "--a",
        action="append",
...
def resolve_check(
    ...
    """Return (a_specs, b_specs, path_kind, check_id). Breakpoint target #2."""
    if args.origins and args.waypoints:
        path_kind = args.path_kind or "ff"
        return list(args.origins), list(args.waypoints), path_kind, "cli"
    ...
    path_kind = args.path_kind or str(map_spec.get("path_kind") or "comb")
    check_id = str(chosen.get("id") or "wp")
    return a_specs, b_specs, path_kind, check_id
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    a_specs, b_specs, path_kind, check_id = resolve_check(args, example_dir)
    print(f"[debug] check_id={check_id} path_kind={path_kind}")
