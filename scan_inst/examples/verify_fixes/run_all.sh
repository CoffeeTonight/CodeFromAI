...
run_case() {
  ...
  local json="$2"
  echo "======== $name ========"
  scan-inst "$json" --index-cwd "$DIR" 2>"/tmp/scan_inst_${name}.stderr" | tail -5
  echo "--- stderr (path-walk highlights) ---"
  rg -n "path-walk|miss inst=|connected|error|ValueError" "/tmp/scan_inst_${name}.stderr" || true
  echo "--- TSV result ---"
  local out
...
scan-inst design.f --top top --mode path-walk --check-connect topo.u topo.u --index-cwd "$DIR" --no-cache 2>&1 | tail -8 || true