# Agent rules — `~/tools/__CFI`

**Mandatory:** Before any write, patch, delete, `rsync`, or destructive shell command under `~/tools/__CFI`, run a snapshot first.
...
```bash
bash ~/tools/__CFI/scripts/cfa_pre_edit.sh --label <short-task-name> -- <your command>
```

- Do **not** set `CFA_SKIP_AUTO_BACKUP=1` unless the user explicitly asks.
- After backup, note the path from `~/tools/__CFI-backups/LATEST_MANUAL_BACKUP.txt` in your progress message when the change is large.
