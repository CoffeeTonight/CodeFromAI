...
def test_effective_low_memory_auto_threshold(monkeypatch):
    monkeypatch.delenv("SCAN_INST_LOW_MEMORY_AUTO", raising=False)
...
def test_effective_low_memory_auto_disabled(monkeypatch):
    monkeypatch.setenv("SCAN_INST_LOW_MEMORY_AUTO", "0")
...
def test_body_param_scan_max_env(monkeypatch):
    ...
    monkeypatch.setenv("SCAN_INST_BODY_PARAM_SCAN_MAX", "0")
    assert body_param_scan_max() == 0
    monkeypatch.setenv("SCAN_INST_BODY_PARAM_SCAN_MAX", "4096")
