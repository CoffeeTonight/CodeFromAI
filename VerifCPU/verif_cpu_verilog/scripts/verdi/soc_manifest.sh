#!/usr/bin/env bash
# Open Test — bridge wiring reference (tb_soc_manifest) in Synopsys Verdi (source + optional VCD).
# Requires: ./example.sh gen  (generated .vh + filelists)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"
VERDI="${VERDI:-verdi}"
TOP="${VERDI_TOP:-tb_soc_manifest}"
FLIST="filelists/test/verdi_soc_manifest.f"
WAVE="${1:-}"
...
DEFAULT_VCD="sim_build/tb_soc_manifest.vcd"
