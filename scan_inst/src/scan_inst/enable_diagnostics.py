...
def audit_run_on_full_index_enable_lines(
    ...
    *,
    ...
    """
    from scan_inst.run_request import _full_index_block_key
...
def format_enable_root_cause_hint(
    *,
    ...
    if not saw_suite_trace:
        return (
            "ROOT CAUSE: this scan-inst binary did not execute flat-suite parsing "
            f"(no enable-trace: block= lines; version={version}). "
            "enable:0 in JSON is ignored on that code path — reinstall so "
            "scan-inst --version shows this package directory, not an old "
            "site-packages build."
        )
