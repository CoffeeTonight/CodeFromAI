...
> 신호·매크로·RTL 상세: [`howto_integrate.md`](howto_integrate.md), [`vcpu_skill.md`](vcpu_skill.md).  
> (선택) soc-verify-agent 워크플로: 별도 repo `projects/VERIF-CPU-SOC/USER-PROCEDURE.md`.

**대상 독자:** SoC RTL/검증 엔지니어 — 주소맵·interconnect 포트명을 알고 있으며, soc-verify-agent gate(c-compile → coi_conn → slave_rw)로 통합을 검증하려는 사람.

> **사용자가 인간임을 잊지 않도록** — 포팅은 **YAML·manifest만** 편집하고 TB를 슬레이브별로 하드코딩하지 않습니다. `make icodes` + `make bus_connect_yaml`(또는 `gen_soc_bus_connect.py --yaml`) 후 smoke sim log를 **직접 열어** PASS를 eye-check 하세요.
...
## 2. 현업 참조 시나리오 (4-slave mixed-bus)

`inputs/tags/main/deployment/customer_soc_intake.example.yaml`이 **일반적인 현업 SoC DV 통합** 참고 SSOT입니다.

| Slave | Protocol | bus_port | addr_base | cpu_id | tap_port | enabled |
|-------|----------|----------|-----------|--------|----------|---------|
| SFR | APB | `S01_APB` | `0x4000_0000` | 1 | 0 | 1 |
| SRAM | AHB-Lite | `M02_AHB` | `0x8000_0000` | 2 | 1 | 1 |
| UART | AXI4-Lite | `S03_AXI` | `0xC000_0000` | 3 | 2 | 1 |
| DMA_CH3 | AXI4-Lite | `S37_AXI` | `0x4A00_0000` | 37 | 36 | 0 (reserved) |

실무 흐름: `cp soc_hierarchy_example.yaml` → `bus_port` / `addr_base` / `cpu_id` **수동 편집** → `make icodes` → `make bus_connect_yaml` (또는 `--yaml` 칩 파일) → top 배선 → smoke log eye-check.

---

## 3. 내 SoC에서 수집할 정보 (slave 1개당)
...
현업 4-slave(SFR/SRAM/UART/DMA) 예시는 intake example을 따르며, 포트명·주소는 **RTL PDF·주소맵과 1:1 double-check** (eye-check).
...
`integration_notes.md`, `questions_pending.md`, `customer_soc_intake.yaml` 등 — `soc-verify-agent/templates/obsidian/agent/vcpu-soc-integration/12-EXAMPLE-SCAFFOLD.md` (별도 clone 필요).
...
**검증 주소가 나뉜 C/헤더는 사용자가 준비**한다. 에이전트는 경로를 **사용자에게 질문**한 뒤, 받은 C 다발을 `firmware/campaign/` **정해진 위치에 복사**하고 `campaign_slots.yaml`·`NUM_SCPU`를 맞춘다 — `soc-verify-agent/templates/obsidian/agent/vcpu-soc-integration/10-FIRMWARE-STAGE.md`.
...
템플릿: `soc-verify-agent/templates/obsidian/agent/vcpu-soc-integration/intake/simulation_env.template.yaml` · 가이드: `soc-verify-agent/templates/obsidian/agent/vcpu-soc-integration/11-SIMULATION-USER.md`
...
### Step 4 — connect VH 생성 (yaml만, TB 하드코딩 금지)

```bash
cd firmware/campaign
# 칩 전용 yaml SSOT — 슬레이브별 TB 수정 없음
python3 gen_soc_bus_connect.py --yaml soc_hierarchy_<MY_CHIP>.yaml
# 예제 yaml 복사본 그대로면: make bus_connect_yaml
# (manifest-only 레이아웃일 때만: make bus_connect)
# 산출: include/verif_soc_bus_connect.vh
```

생성된 `CONNECT_SLVxx_*` 매크로의 `Sxx_*` prefix가 **과제 top 포트명**과 문자열 일치해야 한다 — RTL diagram과 **port match eye-check**.  
상세 매크로 형식 → `$RTL_ROOT/howto_integrate.md` §5.3.

### Step 5 — 과제 top에 셀·agent·CONNECT 삽입 (yaml 생성물만 include)
...
Agent/LLM 자동화 시 → `$RTL_ROOT/vcpu_skill.md` · `soc-verify-agent/templates/obsidian/agent/vcpu-soc-integration/00-INTEGRATION-HUB.md`.
...
# eye-check: sim_smoke.log를 열어 PASS 줄을 직접 확인 (사용자가 인간임을 잊지 않도록)
```
...
| chip-top 12-check OK, 내 top FAIL | 예제 yaml 그대로 사용 | **내 yaml** + 내 top으로 Step 1~5 재수행 |
