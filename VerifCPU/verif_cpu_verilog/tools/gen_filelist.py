...
def _emit_verdi_combined(source_flist: str, *, top: str) -> str:
    ...
    lines: list[str] = [
        ...
        f"// Top: {top}",
        "// Generated by tools/gen_filelist.py — do not edit",
        "// Usage (package root):  verdi -sv -f filelists/<work|test>/verdi_<view>.f -top <top>",
        "",
        *INCDIRS,
        "",
    ]
...
def _emit_vcs_run() -> str:
    return "\n".join([
        ...
        "set -e",
        'if [[ "$st" -ne 0 ]]; then',
        '  echo "[vcs] sim failed exit=$st (log=$OUTDIR/sim.log)" >&2',
        '  exit "$st"',
        "fi",
...
def _emit_verdi_readme() -> str:
    ...
    for v in VERDI_VIEWS:
        ...
        cat = v.get("category", "test")
        lines.append(
            f"#   verdi -sv -f filelists/{cat}/verdi_{v['id']}.f -top {v['top']}{wave}"
        )
    lines.extend([
        "",
