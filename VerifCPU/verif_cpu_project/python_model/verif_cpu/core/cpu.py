...
class VerifCPU:
    ...
    def step(self):
        ...
        if self.sync_pending:
            from verif_cpu.core.sync_barrier import global_sync_barrier

            self.total_steps += 1
            hub = self.sync_barrier or global_sync_barrier
            if hub.cpu_consume_go(self.cpu_id):
                self.sync_pending = False
                self.sync_hold_pc = False
                self.state = CPUState.RUNNING
                self.pc += 4
    ...
    def _simple_disasm(self, inst, raw: int) -> str:
        ...
        if op == 0x13:  # OP_IMM
            ...
            if f3 == 1: return f"slli x{inst.rd},x{inst.rs1},{inst.imm & 0x1f}"
            if f3 == 2: return f"slti x{inst.rd},x{inst.rs1},{inst.imm}"
            if f3 == 3: return f"sltiu x{inst.rd},x{inst.rs1},{inst.imm}"
            if f3 == 5:
                return f"srai x{inst.rd},x{inst.rs1},{inst.imm & 0x1f}" if inst.funct7 == 0x20 else f"srli x{inst.rd},x{inst.rs1},{inst.imm & 0x1f}"
        ...
        if op == 0x17:
            return f"auipc x{inst.rd},0x{inst.imm >> 12:x}"
        if op == 0x0B:
        ...
        if op == 0x63:  # BRANCH
            ...
            if inst.funct3 == 4: return f"blt x{inst.rs1},x{inst.rs2},0x{inst.imm:x}"
            if inst.funct3 == 5: return f"bge x{inst.rs1},x{inst.rs2},0x{inst.imm:x}"
            if inst.funct3 == 6: return f"bltu x{inst.rs1},x{inst.rs2},0x{inst.imm:x}"
            if inst.funct3 == 7: return f"bgeu x{inst.rs1},x{inst.rs2},0x{inst.imm:x}"
