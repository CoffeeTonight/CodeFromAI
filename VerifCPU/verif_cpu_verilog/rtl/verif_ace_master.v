...
module verif_ace_master #(
  ...
  verif_axi_full_master #(
    ...
    .ARPROT(ARPROT), .ARLOCK(), .ARQOS(ARQOS), .ARREGION(ARREGION), .ARVALID(ARVALID), .ARREADY(ARREADY),
    .RID(RID), .RDATA(RDATA), .RRESP(RRESP), .RLAST(RLAST), .RVALID(RVALID), .RREADY(RREADY),
    .AWID(AWID), .AWADDR(AWADDR), .AWLEN(AWLEN), .AWSIZE(AWSIZE), .AWBURST(AWBURST),
    .AWPROT(AWPROT), .AWLOCK(), .AWQOS(AWQOS), .AWREGION(AWREGION), .AWATOP(AWATOP), .AWVALID(AWVALID), .AWREADY(AWREADY),
