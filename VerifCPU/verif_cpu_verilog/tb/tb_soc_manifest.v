// Integration TB — manifest-driven fabric (generated tb_soc_manifest_gen.vh)
// Phase A/B/C through real AMBA bridges (not simple_soc task bus).
// SSOT: campaign_slots.yaml + soc_hierarchy*.yaml → make icodes
...
module tb_soc_manifest;
  ...
  localparam integer TB_EXPECTED_PASS = 24;
  ...
  initial begin
    ...
    $display("========================================================================");
    $display("tb_soc_manifest: manifest-driven CONNECT_SLV* + Phase A/B/C (%0d slaves)",
             `SOC_MANIFEST_NUM_SLAVES);
    $display("========================================================================");
    ...
    if (pass != TB_EXPECTED_PASS)
      $fatal(1, "tb_soc_manifest: pass=%0d expected %0d", pass, TB_EXPECTED_PASS);
