...
from scan_inst.cache import build_design_index
from scan_inst.elab import elaborate
from scan_inst.filelist import parse_filelist
from scan_inst.lazy_scope import elab_scope_paths
from scan_inst.perf import include_warm_enabled
from scan_inst.preprocess import _warm_include_cache_for_sources
...
def test_include_warm_opt_in_off_by_default(monkeypatch, tmp_path: Path):
    monkeypatch.delenv("SCAN_INST_INCLUDE_WARM", raising=False)
    monkeypatch.delenv("SCAN_INST_NO_INCLUDE_WARM", raising=False)
    ...
    assert warmed == 0
    assert any("SCAN_INST_INCLUDE_WARM=1" in line for line in lines)
