...
class CompanyLLM(CustomLLM):
    ...
    def complete(self, prompt: str, **kwargs) -> CompletionResponse:
        ...
        payload = {
            ...
            # 회사 API에 맞는 다른 파라미터 추가
        }

        try:
            response = httpx.post(self.api_url, json=payload, headers=headers, timeout=180)
            response.raise_for_status()
            data = response.json()

            # 회사 API 응답 형식에 맞게 수정 (예시)
            # 실제 응답 구조 확인 후 수정하세요
...
class RAGEngine:
    def __init__(self):
        ...
        if llm_type == "ollama":
            ...
            self.llm = CompanyLLM()
            logger.info(f"[LLM] 회사 LLM 사용 (Custom): {current_config['model']} ({current_config['api_url']})")
        else:
...
"""
    ...
    def query(self, question: str) -> str:
        """외부에서 호출하는 공용 쿼리 함수 (회사 LLM 호환)"""
        return self.manual_rag_query(question, top_k=15)
