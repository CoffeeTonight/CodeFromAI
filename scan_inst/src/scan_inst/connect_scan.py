...
def prepare_connect_body(
    ...
    *,
    ...
    from scan_inst.preprocess import _collect_define_undef, _expand_macros, apply_ifdef_filter
...
def _parse_assign_stmt(
    ...
    *,
    ...
    if hier_links is not None and hier_ref_targets is not None:
        if extract_hier_refs(lhs) or extract_hier_refs(rhs):
...
def _apply_ifdef_only(body: str, defines: Mapping[str, str] | None) -> str:
    if not defines:
        return body
    from scan_inst.preprocess import apply_ifdef_filter
...
def collect_design_defines(index: object) -> Dict[str, str]:
    ...
    from scan_inst.preprocess import _collect_define_undef
...
def build_module_connect_index(
    ...
    *,
    ...
    for base, bits in decl_widths.items():
        ...
        if len(bits) > 1:
            msb, lsb = max(bits), min(bits)
            bus_nodes.append(f"{base}[{msb}:{lsb}]")
        for a in bus_nodes:
            for b in bus_nodes:
                _add_undirected(assign_adj, a, b)
