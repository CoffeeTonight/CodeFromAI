...
class VSync(CustomInstruction):
    """Multi-CPU sync barrier (VSEL_SYNC / 0x13)."""
    def __init__(self):
    ...
    def execute(self, cpu, rd, rs1, rs2, imm):
        ...
        sync_id = (rd & 0xFF) if rd else 0
        if hasattr(cpu, 'log'):
            cpu.log(f"vsync id=0x{sync_id:02x}")
        else:
            print(f"SCPU{cpu.cpu_id} > vsync id=0x{sync_id:02x}")

        hub = getattr(cpu, "sync_barrier", None) or global_sync_barrier
        cpu.sync_hold_pc = True
        cpu.sync_pending = True
        cpu.state = "STALLED"
        hub.cpu_arrive(cpu.cpu_id, sync_id)
...
class VAssert(CustomInstruction):
    ...
    def execute(self, cpu, rd, rs1, rs2, imm):
        condition = cpu.read_reg(rs1) if rs1 else cpu.read_reg(1)
        assert_id = rd if rd else imm
        passed = bool(condition)

        if passed:
            if hasattr(cpu, 'log'):
                cpu.log(f"[Assert] PASS (id={assert_id})")
        else:
            msg = f"ASSERTION FAILED (id={assert_id})"
            if hasattr(cpu, 'log'):
        ...
        if getattr(cpu, 'coverage_collector', None):
            cpu.coverage_collector.record_assert(assert_id, passed)
...
class VForce(CustomInstruction):
    ...
    def execute(self, cpu, rd, rs1, rs2, imm):
        # verif_insns.h: vforce(target_rd, value_rs2) → rd=target, rs2=value
        target = cpu.read_reg(rs1) if rs1 else rd
        value = cpu.read_reg(rs2) if rs2 else imm
...
def encode_auipc(rd: int, imm20: int) -> int:
    ...
    return ((imm20 & 0xFFFFF) << 12) | ((rd & 0x1F) << 7) | OPCODE_AUIPC
